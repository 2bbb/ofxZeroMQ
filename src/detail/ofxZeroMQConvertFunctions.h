...
#pragma mark ofBuffer
        inline static void to_zmq_message(Message &m,
                                          const ofBuffer &data)
        { m.rebuild(data.getData(), data.size()); };

        inline static void from_zmq_message(const Message &m,
                                            ofBuffer &data)
        {
            data.resize(m.size());
...
#pragma mark ofPixels
                                          ...
                                          const ofPixels_<pix_type> &pix)
        {
            ...
            ofPixelFormat pixel_format = pix.getPixelFormat();
            
            m.rebuild(sizeof(uint32_t) * 2 + sizeof(ofPixelFormat) + sizeof(pix_type) * pix.size());
            auto offset = m.copyFrom(size);
            offset += m.copyFrom(pixel_format, offset);
            m.memCopyFrom(pix.getData(), sizeof(pix_type) * pix.size(), offset);
        }
                                            ...
                                            ofPixels_<pix_type> &pix)
        {
            ...
            auto offset = m.copyTo(size);
            offset += m.copyTo(pixel_format, offset);
